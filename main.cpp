    ...
    int main() {
        ...
        bool tieVisible = true; // Flaga do kontrolowania widocznoœci TIE Fighter
        ...
        float gwiazdaSpeed = 150.0f; // Ustaw prêdkoœæ gwiazdy
        ...
        //Zmienna do kontrolowania aktywnoœci TIE Fightera
        bool tieActive = true;
        //Zmienna do œledzenia liczby kolizji gwiazdy z pociskami
        int kolizjeGwiazdaPociski = 0;

        bool startHardMode = false;  // Zmienna do przechowywania stanu trybu trudnego

        bool confirmClose = false; // Flaga do wyœwietlania komunikatu potwierdzenia zamkniêcia gry
        ...
        while (window.isOpen()) {
            ...
            while (window.pollEvent(event)) {
                ...
                // Obs³uga anulowania zamkniêcia gry
                    ...
                    if (gameState == GameState::Menu) {
                        ...
                        if (event.type == sf::Event::MouseButtonPressed) {
                            if (gameState == GameState::Menu) {
                                ...
                                // SprawdŸ, czy myszka zosta³a klikniêta na przycisku "START TRUDNY"
                                if (startHardButton.getGlobalBounds().contains(static_cast<float>(mousePosition.x), static_cast<float>(mousePosition.y))) {
                                    ...
                                    tieSpeed = startHardMode ? 300.0f : 150.0f;  // Zwiêksz prêdkoœæ dla trybu trudnego
                        ...
                        if (showButtons && startButton.getGlobalBounds().contains(static_cast<float>(mousePosition.x), static_cast<float>(mousePosition.y))) {
                            ...
                            // W sekcji obs³ugi zdarzeñ myszy (po obs³udze przycisku START):
                            if (showButtons && startHardButton.getGlobalBounds().contains(static_cast<float>(mousePosition.x), static_cast<float>(mousePosition.y))) {
                                ...
                                tieTarget = sf::Vector2f(rand() % window.getSize().x, rand() % window.getSize().y);
                                czasGry = 0.0f;
                                tieSpeed = 300.0f; // Podwójna prêdkoœæ dla TIE Fightera
                                gwiazdaSpeed = 300.0f; // Podwójna prêdkoœæ dla gwiazdy
                                // Dodaj inne resetowane parametry gry, jeœli s¹
                            }
                        }
           ...
           if (gameState == GameState::Playing && gameStarted && !koniecGry && !isPaused) {
                ...
                for (auto it = blasters.begin(); it != blasters.end();) {
                    // SprawdŸ, czy liczba punktów wynosi co najmniej 10 przed obs³ug¹ kolizji z gwiazd¹
                    if (punkty >= 10) {
                        if (it->sprite.getGlobalBounds().intersects(gwiazdaSprite.getGlobalBounds())) {
                            // Kolizja z gwiazd¹! Usuñ pocisk i zwiêksz liczbê kolizji
                            it = blasters.erase(it);
                            kolizjeGwiazdaPociski++;

                            // Jeœli liczba kolizji osi¹gnê³a 5, zakoñcz grê
                            if (kolizjeGwiazdaPociski >= 5) {
                                gameState = GameState::Menu; // Powróæ do menu
                        ...
                        ++it; // Jeœli mniej ni¿ 10 punktów, pomiñ logikê kolizji z gwiazd¹
                    }
                }

                // Dodaj zmienn¹, która œledzi, czy gra jest wygrana
                bool graWygrana = false;
                // W pêtli gry, w sekcji obs³uguj¹cej ruch gwiazdy, zaktualizuj logikê gwiazdy
                if (kolizjeGwiazdaPociski < 5 && !graWygrana) {
                    ...
                    // Jeœli osi¹gniêto 5 kolizji, oznacz grê jako wygran¹
                    graWygrana = true;
                }
                ...
                if (punkty >= 10 && tieActive) {
                    tieActive = false; // Dezaktywuj TIE Fighter po osi¹gniêciu 10 punktów
                    tieVisible = false; // Ukryj TIE Fighter
                }
            ...
            if (gameState == GameState::Menu) {
                ...
                // Rysowanie elementów w trakcie gry
